{{-- resources/views/petugas/keluar.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Kendaraan Keluar - ParkirKu</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Inter:opsz,wght@14..32,400;14..32,500;14..32,600;14..32,700;14..32,800&family=Courier+Prime:wght@700&display=swap" rel="stylesheet">
    {{-- html5-qrcode dimuat SEKALI dari CDN yang stabil --}}
    <script src="https://cdn.jsdelivr.net/npm/html5-qrcode@2.3.8/html5-qrcode.min.js"></script>
    <style>
        body { font-family: 'Inter', sans-serif; background: linear-gradient(145deg, #f0f4f8 0%, #e2e8f0 100%); }
        @keyframes fadeInUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        .animate-fade-up { animation: fadeInUp 0.5s ease-out forwards; }
        .overlay { transition: opacity 0.3s ease; opacity: 0; visibility: hidden; }
        .overlay.active { opacity: 1; visibility: visible; }
        @media (max-width: 768px) {
            .sidebar { transform: translateX(-100%); position: fixed; z-index: 100; top: 0; left: 0; width: 280px; height: 100vh; box-shadow: 2px 0 20px rgba(0,0,0,0.2); }
            .sidebar.open { transform: translateX(0); }
            .overlay { z-index: 90; }
        }
        .scan-input { font-family: 'Courier Prime', monospace; font-size: 18px; letter-spacing: 0.08em; text-transform: uppercase; }
        @keyframes successPop { 0%{transform:scale(0.8);opacity:0} 70%{transform:scale(1.05)} 100%{transform:scale(1);opacity:1} }
        .success-pop { animation: successPop 0.4s ease forwards; }

        /* Scanner area */
        #qr-reader {
            border-radius: 12px;
            overflow: hidden;
            background: #0f172a;
        }
        /* Override html5-qrcode default styling */
        #qr-reader img { display: none !important; }
        #qr-reader__header_message { display: none !important; }
        #qr-reader__status_span { display: none !important; }
        #qr-reader__camera_selection { display: none !important; }
        #qr-reader__dashboard { display: none !important; }

        /* Scan frame overlay */
        .scan-overlay {
            position: relative;
            background: #0f172a;
            border-radius: 12px;
            overflow: hidden;
        }
        .scan-corners {
            position: absolute; inset: 0; pointer-events: none; z-index: 10;
        }
        .scan-corners::before, .scan-corners::after {
            content: '';
            position: absolute;
            width: 28px; height: 28px;
            border-color: #f97316;
            border-style: solid;
        }
        .scan-corners::before { top: 12px; left: 12px; border-width: 3px 0 0 3px; border-radius: 4px 0 0 0; }
        .scan-corners::after  { bottom: 12px; right: 12px; border-width: 0 3px 3px 0; border-radius: 0 0 4px 0; }
        .scan-line {
            position: absolute;
            left: 12px; right: 12px; height: 2px;
            background: linear-gradient(90deg, transparent, #f97316, transparent);
            animation: scanLine 2s ease-in-out infinite;
            z-index: 9;
        }
        @keyframes scanLine { 0%{top:20%} 50%{top:75%} 100%{top:20%} }

        /* Status badge */
        .status-badge {
            display: flex; align-items: center; gap: 8px;
            padding: 8px 14px;
            border-radius: 10px;
            font-size: 12px; font-weight: 500;
        }
        .status-badge.ready    { background: #fef3c7; color: #92400e; }
        .status-badge.active   { background: #dcfce7; color: #166534; }
        .status-badge.detected { background: #dbeafe; color: #1e40af; }
        .status-badge.error    { background: #fee2e2; color: #991b1b; }
        .status-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
        .status-badge.ready  .status-dot { background: #f59e0b; }
        .status-badge.active .status-dot { background: #22c55e; animation: pulse 1s infinite; }
        .status-badge.detected .status-dot { background: #3b82f6; }
        .status-badge.error    .status-dot { background: #ef4444; }
        @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
    </style>
</head>
<body>
    @include('petugas.partials.navbar')
    <div id="overlay" class="overlay fixed inset-0 bg-black/50 z-50 lg:hidden"></div>
    <div class="flex pt-16 min-h-screen">
        @include('petugas.partials.sidebar')

        <main class="flex-1 p-4 sm:p-6 lg:p-8 overflow-x-hidden">
            <div class="animate-fade-up">

                <div class="mb-6">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Kendaraan Keluar</h1>
                    <p class="text-gray-500 text-sm mt-1">Scan barcode karcis atau input kode secara manual</p>
                </div>

                {{-- Alert Sukses --}}
                @if(session('sukses_keluar'))
                @php $p = \App\Models\Parking::where('ticket_code', session('sukses_keluar'))->first(); @endphp
                @if($p)
                <div class="success-pop bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-5 mb-6 flex items-start gap-4">
                    <div class="w-12 h-12 bg-emerald-500 rounded-full flex items-center justify-center flex-shrink-0">
                        <i class="fas fa-check text-white text-xl"></i>
                    </div>
                    <div class="flex-1">
                        <p class="font-bold text-emerald-700 text-lg">Kendaraan Berhasil Keluar!</p>
                        <p class="text-emerald-600 font-mono text-sm mt-1">{{ $p->no_kendaraan }} — {{ strtoupper($p->jenis_kendaraan) }}</p>
                        <div class="mt-3 grid grid-cols-3 gap-3">
                            <div class="bg-white rounded-xl p-3 text-center border border-emerald-200">
                                <p class="text-xs text-gray-500">Masuk</p>
                                <p class="font-bold text-gray-800 text-sm">{{ $p->waktu_masuk->format('H:i') }}</p>
                            </div>
                            <div class="bg-white rounded-xl p-3 text-center border border-emerald-200">
                                <p class="text-xs text-gray-500">Durasi</p>
                                <p class="font-bold text-gray-800 text-sm">{{ $p->durasi }} mnt</p>
                            </div>
                            <div class="bg-white rounded-xl p-3 text-center border border-emerald-200">
                                <p class="text-xs text-gray-500">Total</p>
                                <p class="font-bold text-emerald-600 text-sm">Rp {{ number_format($p->tarif, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                @endif
                @endif

                {{-- Alert Error --}}
                @if(session('error'))
                <div class="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 mb-5 flex items-center gap-3 text-sm">
                    <i class="fas fa-exclamation-circle text-red-500"></i>
                    <span>{{ session('error') }}</span>
                </div>
                @endif

                <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">

                    {{-- Panel Scan --}}
                    <div class="lg:col-span-2">
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">

                            {{-- Header --}}
                            <div class="bg-gradient-to-r from-orange-500 to-red-500 p-4 sm:p-5">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center">
                                        <i class="fas fa-barcode text-white text-lg"></i>
                                    </div>
                                    <div>
                                        <h2 class="font-bold text-white">Scan / Input Karcis</h2>
                                        <p class="text-orange-100 text-xs">Arahkan kamera ke barcode pada karcis</p>
                                    </div>
                                </div>
                            </div>

                            <div class="p-5 space-y-4">

                                {{-- Status Badge --}}
                                <div id="statusBadge" class="status-badge ready">
                                    <span class="status-dot"></span>
                                    <span id="statusText">Siap menerima scan atau input manual</span>
                                </div>

                                {{-- Form Input Manual --}}
                                <form action="{{ route('petugas.keluar.proses') }}" method="POST" id="keluarForm">
                                    @csrf
                                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Kode Karcis</label>
                                    <div class="flex gap-2">
                                        <input type="text" name="ticket_code" id="ticketInput"
                                            placeholder="Scan atau ketik kode..."
                                            class="scan-input flex-1 border-2 border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
                                            autocomplete="off" autofocus>
                                        <button type="submit"
                                            class="bg-orange-500 hover:bg-orange-600 text-white font-bold px-4 py-3 rounded-xl transition shadow flex items-center gap-2 whitespace-nowrap">
                                            <i class="fas fa-sign-out-alt"></i>
                                        </button>
                                    </div>
                                    <p class="text-xs text-gray-400 mt-1.5">Contoh: PKR-20250403-AB123</p>
                                </form>

                                {{-- Divider --}}
                                <div class="flex items-center gap-3">
                                    <hr class="flex-1 border-gray-200">
                                    <span class="text-xs text-gray-400 font-medium">atau gunakan kamera</span>
                                    <hr class="flex-1 border-gray-200">
                                </div>

                                {{-- Tombol Kamera --}}
                                <button id="toggleCameraBtn" type="button"
                                    class="w-full bg-blue-50 hover:bg-blue-100 border-2 border-blue-200 text-blue-700 font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 text-sm">
                                    <i class="fas fa-camera"></i> <span id="cameraBtnText">Buka Kamera Scanner</span>
                                </button>

                                {{-- Area Scanner --}}
                                <div id="scannerWrap" style="display:none">
                                    <div class="scan-overlay">
                                        <div class="scan-corners"></div>
                                        <div class="scan-line" id="scanLine"></div>
                                        <div id="qr-reader" style="width:100%;height:220px"></div>
                                    </div>
                                    <p class="text-xs text-center text-gray-500 mt-2">
                                        <i class="fas fa-info-circle mr-1"></i>
                                        Arahkan barcode CODE128 horizontal ke tengah area merah
                                    </p>
                                </div>

                            </div>
                        </div>
                    </div>

                    {{-- Riwayat Keluar --}}
                    <div class="lg:col-span-3">
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            <div class="p-4 sm:p-5 border-b border-gray-100 flex items-center justify-between">
                                <h3 class="font-semibold text-gray-800">Riwayat Keluar Hari Ini</h3>
                                <span class="px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-semibold">{{ $riwayat->count() }} terakhir</span>
                            </div>
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm">
                                    <thead>
                                        <tr class="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider text-left">
                                            <th class="px-4 py-3">No. Kendaraan</th>
                                            <th class="px-4 py-3">Jenis</th>
                                            <th class="px-4 py-3">Masuk</th>
                                            <th class="px-4 py-3">Keluar</th>
                                            <th class="px-4 py-3">Total</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-100">
                                        @forelse($riwayat as $p)
                                        <tr class="hover:bg-gray-50 transition">
                                            <td class="px-4 py-3"><span class="font-bold text-gray-800 tracking-wider text-xs">{{ $p->no_kendaraan }}</span></td>
                                            <td class="px-4 py-3">
                                                @php $icon = match($p->jenis_kendaraan){'mobil'=>'fa-car','truk'=>'fa-truck',default=>'fa-motorcycle'}; @endphp
                                                <i class="fas {{ $icon }} text-gray-400"></i>
                                            </td>
                                            <td class="px-4 py-3 text-gray-500 text-xs">{{ $p->waktu_masuk->format('H:i') }}</td>
                                            <td class="px-4 py-3 text-gray-500 text-xs">{{ $p->waktu_keluar?->format('H:i') ?? '-' }}</td>
                                            <td class="px-4 py-3"><span class="text-xs font-semibold text-emerald-600">Rp {{ number_format($p->tarif ?? 0, 0, ',', '.') }}</span></td>
                                        </tr>
                                        @empty
                                        <tr><td colspan="5" class="px-4 py-10 text-center text-gray-400">
                                            <i class="fas fa-history text-3xl mb-2 block opacity-30"></i>
                                            <p class="text-sm">Belum ada riwayat keluar hari ini</p>
                                        </td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </main>
    </div>
    @include('petugas.partials.footer')

    <script>
        // Sidebar
        const sidebar=document.getElementById('sidebar'),toggleBtn=document.getElementById('sidebarToggle'),overlay=document.getElementById('overlay');
        function closeSidebar(){sidebar.classList.remove('open');overlay.classList.remove('active');}
        function openSidebar(){sidebar.classList.add('open');overlay.classList.add('active');}
        toggleBtn.addEventListener('click',e=>{e.stopPropagation();sidebar.classList.contains('open')?closeSidebar():openSidebar();});
        overlay.addEventListener('click',closeSidebar);
        window.addEventListener('resize',()=>{if(window.innerWidth>768)closeSidebar();});

        // Auto submit saat scanner mengirim Enter (barcode scanner fisik)
        const ticketInput = document.getElementById('ticketInput');
        ticketInput.focus();
        ticketInput.addEventListener('keydown', function(e){
            if(e.key === 'Enter' && this.value.trim().length > 0){
                e.preventDefault();
                document.getElementById('keluarForm').submit();
            }
        });

        // ===== Status Badge =====
        function setStatus(type, text) {
            const badge = document.getElementById('statusBadge');
            badge.className = 'status-badge ' + type;
            document.getElementById('statusText').textContent = text;
        }

        // ===== Kamera Scanner =====
        let scanner = null;
        let cameraOpen = false;
        let scanCooldown = false;

        const toggleBtn2 = document.getElementById('toggleCameraBtn');
        const scannerWrap = document.getElementById('scannerWrap');
        const cameraBtnText = document.getElementById('cameraBtnText');

        toggleBtn2.addEventListener('click', async function() {
            if (!cameraOpen) {
                await startScanner();
            } else {
                await stopScanner();
            }
        });

        async function startScanner() {
            if (typeof Html5Qrcode === 'undefined') {
                setStatus('error', 'Library scanner tidak termuat, refresh halaman');
                return;
            }

            scannerWrap.style.display = 'block';
            cameraBtnText.textContent = 'Tutup Kamera';
            toggleBtn2.classList.replace('bg-blue-50','bg-red-50');
            toggleBtn2.classList.replace('border-blue-200','border-red-200');
            toggleBtn2.classList.replace('text-blue-700','text-red-700');
            toggleBtn2.querySelector('i').className = 'fas fa-times';
            setStatus('active', 'Kamera aktif — arahkan barcode ke tengah');

            try {
                scanner = new Html5Qrcode('qr-reader');

                // Konfigurasi: aktifkan CODE128 dan format lainnya (bukan hanya QR!)
                const config = {
                    fps: 15,
                    qrbox: { width: 260, height: 100 },  // lebar untuk barcode, pendek untuk 1D
                    formatsToSupport: [
                        Html5QrcodeSupportedFormats.CODE_128,
                        Html5QrcodeSupportedFormats.CODE_39,
                        Html5QrcodeSupportedFormats.QR_CODE,
                        Html5QrcodeSupportedFormats.EAN_13,
                        Html5QrcodeSupportedFormats.EAN_8
                    ],
                    experimentalFeatures: {
                        useBarCodeDetectorIfSupported: true  // gunakan native BarcodeDetector jika ada
                    }
                };

                await scanner.start(
                    { facingMode: 'environment' },
                    config,
                    // SUCCESS callback
                    function(decodedText) {
                        if (scanCooldown) return;
                        scanCooldown = true;

                        const code = decodedText.trim().toUpperCase();
                        ticketInput.value = code;
                        setStatus('detected', 'Kode terdeteksi: ' + code);

                        stopScanner().then(() => {
                            setTimeout(() => {
                                document.getElementById('keluarForm').submit();
                            }, 300);
                        });
                    },
                    // ERROR callback — abaikan, ini normal setiap frame gagal
                    function(errorMsg) {
                        // Jangan tampilkan error frame-by-frame ke user
                        // Error "No MultiFormat Readers" adalah normal saat tidak ada kode di frame
                    }
                );

                cameraOpen = true;
            } catch (err) {
                setStatus('error', 'Gagal buka kamera: ' + (err.message || 'Periksa izin kamera'));
                scannerWrap.style.display = 'none';
                resetCameraBtn();
            }
        }

        async function stopScanner() {
            if (scanner) {
                try { await scanner.stop(); } catch(e) {}
                scanner = null;
            }
            cameraOpen = false;
            scannerWrap.style.display = 'none';
            resetCameraBtn();
            if (!scanCooldown) {
                setStatus('ready', 'Siap menerima scan atau input manual');
            }
        }

        function resetCameraBtn() {
            cameraBtnText.textContent = 'Buka Kamera Scanner';
            toggleBtn2.classList.replace('bg-red-50','bg-blue-50');
            toggleBtn2.classList.replace('border-red-200','border-blue-200');
            toggleBtn2.classList.replace('text-red-700','text-blue-700');
            toggleBtn2.querySelector('i').className = 'fas fa-camera';
        }
    </script>
</body>
</html>